...
function gameStart() {
  ...
  document.querySelector(
    "#petName"
  ).innerHTML = `<h2>${PET.stats.name} akachan</h2>`;
  idleDisplay();
...
document.querySelector("#light").addEventListener("click", function () {
  ...
  mainDisplay.innerHTML = `
        <div class=''>
            ...
            <button onClick='lightToggle()' class='nes-btn'>${
              PET.stats.sleep ? "On" : "Off"
            }</button>    
        </div>
    `;
});
...
function guessGame() {
  ...
  const firstNumber = randomSource.splice(
    Math.floor(Math.random() * randomSource.length),
    1
  );
  const secondNumber = randomSource.splice(
    Math.floor(Math.random() * randomSource.length),
    1
  );
  mainDisplay.innerHTML = `  
  ...
  userGuess.addEventListener("click", function (e) {
    ...
    if (target.matches(".nes-btn")) {
      if (
        (secondNumber - firstNumber > 0 && target.dataset.pick == "gt") ||
...
document.querySelector("#stats").addEventListener("click", function () {
  ...
  for (let i = 4; i > 0; i--) {
    htmlTemplate += `<i class='nes-icon is-medium ${
      i > PET.stats.sickness ? "heart" : "heart is-empty"
    }'></i>`;
  }
...
function petAnimation() {
  ...
  let petMoving = setInterval(() => {
    if (
      ...
      document.querySelector(".Character").classList.contains("moving")
    ) {
